...
def calculate_kdj(high, low, close, len_rsv=9, len_k=3, len_d=3):
    ll = low.rolling(len_rsv).min()
    hh = high.rolling(len_rsv).max()
    rsv = np.where(hh == ll, 50.0, ((close - ll) / (hh - ll)) * 100.0)
    rsv = pd.Series(rsv, index=close.index).replace([np.inf, -np.inf], np.nan).fillna(50.0)
    k_val = rsv.ewm(alpha=1 / len_k, adjust=False).mean()
    d_val = k_val.ewm(alpha=1 / len_d, adjust=False).mean()
...
def fetch_stock_name_from_isin_table(str_mode):
    ...
    try:
        with urlopen(make_request(url, "https://isin.twse.com.tw/"), timeout=10) as response:
...
def fetch_twse_stock_name(symbol):
    ...
    for month_start in month_starts:
        ...
        try:
            with urlopen(make_request(url, "https://www.twse.com.tw/"), timeout=10) as response:
...
def scan_signal(df, strict_trend=True, enable_early=True, use_dynamic=True):
    ...
    _, st_dir = calculate_supertrend(high, low, close, period=6, multiplier=0.686)
    is_green_trend = st_dir[-1] == -1

    cci_val = calculate_cci(high, low, close, length=20)
    cci_ma = cci_val.rolling(14).mean()
    cci_tr = (cci_val - cci_val.shift(1).fillna(cci_val)).abs()
    cci_atr = cci_tr.rolling(14).mean()

    final_sens = cci_atr * 1.5 if use_dynamic else pd.Series(30.0, index=cci_val.index)
    final_early = cci_atr * 0.5 if use_dynamic else pd.Series(10.0, index=cci_val.index)

    gap_up = cci_ma - cci_val
    gap_down = cci_val - cci_ma
    is_under_ma = gap_up > 0
    is_over_ma = gap_down > 0
    is_cci_rising = cci_val > cci_val.shift(1)

    allow_buy = is_green_trend if strict_trend else True
    cond_cci_buy = cci_val.iloc[-2] <= cci_ma.iloc[-2] and cci_val.iloc[-1] > cci_ma.iloc[-1]
    cond_cci_early = (
        enable_early
        and bool(is_under_ma.iloc[-1])
        and float(gap_up.iloc[-1]) <= float(final_early.iloc[-1])
        and close.iloc[-1] > open_.iloc[-1]
        and bool(is_cci_rising.iloc[-1])
    )
    was_over_ma = cci_val.iloc[-2] > cci_ma.iloc[-2]
    cond_cci_reentry = (
        bool(is_over_ma.iloc[-1])
        and bool(was_over_ma)
        and bool(is_cci_rising.iloc[-1])
        and close.iloc[-1] > close.iloc[-2]
    )

    cci_buy_now = allow_buy and (cond_cci_buy or cond_cci_reentry)
    cci_buy_soon = allow_buy and (
        cond_cci_early or (bool(is_under_ma.iloc[-1]) and float(gap_up.iloc[-1]) <= float(final_sens.iloc[-1]) and bool(is_cci_rising.iloc[-1]))
    )

    macd_line, macd_sig, macd_hist = calculate_macd(close, fast=12, slow=26, signal=9)
    macd_buy_now = macd_line.iloc[-2] <= macd_sig.iloc[-2] and macd_line.iloc[-1] > macd_sig.iloc[-1]
    macd_buy_soon = (
        macd_hist.iloc[-1] > macd_hist.iloc[-2]
        and macd_line.iloc[-1] <= macd_sig.iloc[-1]
        and macd_hist.iloc[-1] < 0
    )
    macd_ok = macd_buy_now or macd_buy_soon or (macd_line.iloc[-1] > macd_sig.iloc[-1] and macd_hist.iloc[-1] >= 0)

    k_val, d_val, j_val = calculate_kdj(high, low, close, len_rsv=9, len_k=3, len_d=3)
    kdj_buy_now = k_val.iloc[-2] <= d_val.iloc[-2] and k_val.iloc[-1] > d_val.iloc[-1] and k_val.iloc[-1] <= 35.0
    kdj_buy_soon = (
        k_val.iloc[-1] <= 35.0
        and k_val.iloc[-1] > k_val.iloc[-2]
        and d_val.iloc[-1] > d_val.iloc[-2]
        and j_val.iloc[-1] > j_val.iloc[-2]
    )
    kdj_ok = kdj_buy_now or kdj_buy_soon or (k_val.iloc[-1] > d_val.iloc[-1] and k_val.iloc[-1] < 55)

    signal_score = 0
    signal_score += 2 if cci_buy_now else 1 if cci_buy_soon else 0
    signal_score += 2 if macd_buy_now else 1 if macd_buy_soon else 1 if macd_ok else 0
    signal_score += 2 if kdj_buy_now else 1 if kdj_buy_soon else 1 if kdj_ok else 0

    signal_code = 2 if cci_buy_now and macd_ok and kdj_ok else 1 if cci_buy_soon and macd_ok and kdj_ok else 0
    reason_code = 1 if cci_buy_now and cond_cci_buy else 2 if cci_buy_now else 3 if cci_buy_soon else 0

    if signal_code == 0:
        return None

    signal_text = "買點成立" if signal_code == 2 else "即將買點"
    reason_text = (
        "CCI 黃金交叉" if reason_code == 1 else
        "CCI 接續買點" if reason_code == 2 else
        "CCI 提前預判或接近買點"
    )

    cci_state = "CCI 站上均線" if cci_val.iloc[-1] >= cci_ma.iloc[-1] else "CCI 接近均線"
    macd_state = "MACD 轉強" if macd_line.iloc[-1] >= macd_sig.iloc[-1] else "MACD 收斂中"
    kdj_state = "KDJ 轉強" if k_val.iloc[-1] >= d_val.iloc[-1] else "KDJ 接近交叉"

    return {
        "訊號": signal_text,
        "CCI 依據": reason_text,
        "指標狀態": f"{cci_state} / {macd_state} / {kdj_state}",
        "分數": int(signal_score),
        "最新收盤價": round(float(close.iloc[-1]), 2),
        "CCI 數值": round(float(cci_val.iloc[-1]), 2),
        "MACD": round(float(macd_line.iloc[-1]), 2),
        "MACD 訊號": round(float(macd_sig.iloc[-1]), 2),
        "K 值": round(float(k_val.iloc[-1]), 2),
        "D 值": round(float(d_val.iloc[-1]), 2),
        "J 值": round(float(j_val.iloc[-1]), 2),
        "Supertrend 狀態": "🟢 多頭" if is_green_trend else "🔴 空頭",
    }


def check_taiwan_stock(symbol, strict_trend=True, enable_early=True, use_dynamic=True):
...
st.set_page_config(page_title="台股觀察清單掃描器", layout="wide")
st.title("台股觀察清單掃描器｜CCI + MACD + KDJ")

st.sidebar.header("掃描設定")
strict_trend_ui = st.sidebar.checkbox("啟用趨勢保護", value=True)
enable_early_ui = st.sidebar.checkbox("啟用 CCI 提前預判", value=True)
use_dynamic_ui = st.sidebar.checkbox("啟用 CCI ATR 動態範圍", value=True)
max_rows_ui = st.sidebar.slider("最多顯示筆數", min_value=5, max_value=100, value=20, step=5)
...
if st.button("開始掃描"):
    ...
    for idx, sym in enumerate(symbols, 1):
        try:
            result = check_taiwan_stock(
                sym,
                strict_trend=strict_trend_ui,
                enable_early=enable_early_ui,
                use_dynamic=use_dynamic_ui,
            )
            if result:
    ...
    if results:
        df_res = pd.DataFrame(results).sort_values(by=["分數", "訊號"], ascending=[False, True]).head(max_rows_ui)
        st.success(f"掃描完成！找到 {len(df_res)} 檔符合 CCI + MACD + KDJ 條件的股票")
        ...
        st.download_button(
            "下載掃描結果 CSV",
            data=csv_bytes,
            file_name=f"twstock_watch_scan_{datetime.now().strftime('%Y%m%d')}.csv",
            mime="text/csv",
        )
    else:
        st.warning("目前沒有符合 CCI + MACD + KDJ 同步偏多條件的標的。")
